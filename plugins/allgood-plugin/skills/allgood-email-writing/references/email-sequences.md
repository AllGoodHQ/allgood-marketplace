...
The gap between "opted in" and "bought" is where money is made or lost. This reference shows how to build sequences that bridge that gap.
...
**Output format:** Complete email sequences with subject lines, preview text, full copy, send timing, and CTAs. The master skill's Step 5 says where to write them.
...
5. **What voice/brand?** (The master skill's Step 2 resolves this from the supplied brand voice document, if any)
...
## How This Connects to the Other References

This reference decides sequence **architecture** — how many emails, in what
order, on what schedule, each doing one job. It does not teach sentence-level
craft.

- **`direct-response-copy.md`** writes the words inside each email once this
  reference has decided what that email is for. Subject lines, opening lines,
  curiosity gaps, and CTAs all come from there.

**The flow:** plan the sequence here → write each email with
`direct-response-copy.md` → check the result against The Test below.
